...
struct AhoCorasick {
    ...
    void insert(string& s, int idx, vector<int> &vnum) {
        int node = 0;
        for (char c : s) {
            int j = c - 'a';
...
int main() {
    ...
    int n; cin >> n;
    AhoCorasick aut; 
    vector<int> vertex_num(n);  
    for (int i = 0; i < n; i++) {
        string s; cin >> s;
        aut.insert(s, i, vertex_num);
    }
    aut.resolve_links();
    cout << aut.trie.size() << '\n';
    for (int i = 1; i < sz(aut.trie); i++) {
        cout << aut.trie[i].parent << ' ' << aut.trie[i].fail << '\n';
    }
    for (int x: vertex_num) cout << x << ' ';
}
