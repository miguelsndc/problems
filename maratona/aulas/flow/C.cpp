...
struct Dinic {
    ...
    void add_edge(int u, int v, ll cap, bool rev = false) {
        ...
        adj[v].push_back({u, (int)adj[u].size() - 1, rev ? cap : 0});
    }
...
void bop() {  
    ...
    for (int i = 0; i < m; i++) {
        int a, b; cin >> a >> b;
        d.add_edge(a, b, 1, true);
    }
    int k = d.flow(S, T);
    vector<bool> reach = d.get_reachable(S);
    vector<pair<int,int>> cut;
    for (int u = 1; u <= n; u++) {
        for (auto &[v, rev, cap]: d.adj[u]) {
